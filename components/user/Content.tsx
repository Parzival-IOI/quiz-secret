...
const Content = (props: {data: usersResponse | null, loadData: Function}) => {
  return (
    <div className="flex flex-col gap-1 text-sm px-1">
      <strong className="w-full py-2 px-4 dark:bg-slate-600 bg-slate-500 text-white rounded-t-lg grid grid-cols-5 gap-1 sticky">
        <div>Name</div>
        <div>Email</div>
        <div>Role</div>
        <div>Date</div>
      ...
      </strong>
      {
        props.data?.map((d, index)=> {
          ...
          return (
            <div key={index} className="w-full py-2 px-4 dark:bg-slate-600 bg-slate-500 text-white rounded-lg grid grid-cols-5 gap-1">
