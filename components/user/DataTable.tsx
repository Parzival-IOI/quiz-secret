...
import { order, pageSize, orderByUser } from "@/libs/data"
import { usersResponse, tableResponse } from "@/libs/definition"
...
const DataTable = (props: {fetchTable: Function, api: string}) => {
  ...
  const [orderBy, setOrderBy] = useState<string>(orderByUser.DATE);
  ...
  return (
    <div className="h-full w-full flex flex-col justify-between items-center">
      <div className="w-full flex justify-center items-center px-4 py-2 gap-2 sm:gap-4">
        ...
        <OrderBy orderByFunc={orderByFunc} orderBy={orderByUser} />
        <Order orderSortFunc={orderSortFunc} />
        <Search searchPage={searchPage} />
        <CustomButton path="/user/create" label={<><div className="sm:block hidden">Create</div><div className="sm:hidden block"><Plus /></div></>} />
      </div>
