...
const Answer = (props: {id: number, QId: number, handleFormChangeAnswer: Function, removeAnswer: Function}) => {
  const [enabled, setEnabled] = useState(false)
  return (
    <div className="pt-4 px-3 dark:bg-slate-700 bg-slate-400 rounded-xl mt-3 flex">
      
      <div className="w-full" >
        <div className="relative z-0 w-full mb-5 group">
          <input type="text" name={"questions.answers.answer"} id={"answer" + props.id} className="block py-2.5 px-0 w-full text-sm bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required 
      ...
      <div className="my-auto pl-4" >
        <button type="button" className="mt-2 p-1 rounded-lg dark:bg-slate-600 dark:hover:bg-slate-500 hover:bg-slate-300/50 text-slate-200" onClick={() => props.removeAnswer(props.id)}>{<Cross />}</button>
      </div>
